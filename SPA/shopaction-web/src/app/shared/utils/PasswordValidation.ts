export default class PasswordCheck {
    ...
    public hasMiniumLength(password, minLength) {
      return password.length >= minLength;
    }
    // public hasMiniumUniqueChars(password, minUniqueChar) {
    //   let unique = password.filter(function (value, index, self){
    //     return self.indexOf(value) === index;
    //   });
    //   return unique >= minUniqueChar;
    // }
  
    public hasDigit(password) {
      let format = /\d/;
      return format.test(password);
    }
  
    public hasSpecialChar(password) {
      let format = /[ `!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~]/;
      return format.test(password);
    }
    public hasAnyUpperCase(password) {
      let format = /[A-Z]/;
      return format.test(password);
    }
    public hasAnyLowelCase(password) {
      let format = /[a-z]/;
      return format.test(password)
    }
  
    public isValidPassword(password: any, requireLength: number,  requireNonAlphanumeric: boolean, requireLowercase: boolean, requireUppercase: boolean, requireDigit: boolean) {
      let self = this
